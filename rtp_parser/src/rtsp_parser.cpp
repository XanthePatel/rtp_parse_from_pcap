...
rtsp_parser::stream_map rtsp_parser::streams;
rtsp_parser* rtsp_parser::instance(unsigned int src_ip, unsigned short src_port, unsigned int dst_ip, unsigned short dst_port)
{
	...
	if( it == streams.end())
	{
		rtsp_parser *new_stream = new rtsp_parser(src_ip, src_port, dst_ip, dst_port);
...
rtsp_parser::rtsp_parser(unsigned int src_ip, unsigned short src_port, unsigned int dst_ip, unsigned short dst_port)
	: buffer(1024), 
	state(SEARCH_HDR), 
	rpt_pkt_len(0), 
...
void rtsp_parser::put_data(const char *payload, unsigned int len, struct timeval *capture_time)
{
	buffer.write_data(payload, len);
	search_frame(capture_time);
}

void rtsp_parser::search_frame(struct timeval *capture_time)
{
	int ret;
	bool frm_end = false;
	...
	switch(state)
	{
		case SEARCH_HDR:
			...
			ret = buffer.kmp_search_ring_buffer();  //寻找rtsp包头“$” 0x24
			COMMON_LOG("ret = buffer.kmp_search_ring_buffer: %d", ret);
			if ( ret == -1)
			{//head not find
...
/*
bool rtsp_parser::check_big_endian()
{ 
    union { 
        uint i; 
        char c[4]; 
...
void rtsp_parser::write_frm_info()
{
...
unsigned int rtsp_parser::time_diff(struct timeval *start, struct timeval *end)
{
